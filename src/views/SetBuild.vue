    ...
    <div>
      ...
      <div style="margin: 30px 10px">
        ...
        <div v-show="data.heightArr.length == 3" class="pointList">
          最低点：<el-input v-model="data.heightArr[0]" /> 分割点：<el-input
...
const drawpolyline = () => {
  ...
  data.unitArr = [
    [polygonAllGeojson.geometry.coordinates.toString(), 1, polygonEntity],
  ];
  ...
  toDraw(global.$viewer, "line", (res) => {
    ...
    Cesium.GeoJsonDataSource.load(polygonGeojsonArr, {
      clampToGround: true,
    }).then((info) => {
      global.$viewer.entities.removeAll();
      data.unitArr = [];
